...
def FatTreeNet(args, k=4, bw=100, cpu=-1, queue=100, controller='DCController'):
...
def iperfTrafficGen(args, hosts, net):
    ...
    for line in f: 
        ...
        d_out = dest_host.cmd("iperf3 -s &")
        s_out = src_host.cmd("iperf3 -c "+ dest +" -t 10 -P 8")
        dest_host.cmd("pkill iperf3")
        src_host.cmd("pkill iperf3")

        alas += 1
        output1 = open(args.output_dir+"h"+str(alas)+".out",'a')
        output1.write(args.output_dir)
        output1.write(s_out)
        output1.close()
